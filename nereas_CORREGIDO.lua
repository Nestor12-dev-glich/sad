...
-- FUNCIONES PARA SEGURIDAD DE ARCHIVOS
local function safeIsfile(path)
    if HS and HS.isfile then
        return HS.isfile(path)
    end
    return false
end

local function safeReadfile(path)
    if HS and HS.readfile then
        return HS.readfile(path)
    end
    return nil
end

local function safeWritefile(path, content)
    if HS and HS.writefile then
        HS.writefile(path, content)
    end
end
